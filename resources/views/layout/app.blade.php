    ...
    <div class="flex items-center justify-between">
        <nav class="flex space-x-reverse space-x-4">
            ...
            <a href="{{ route('attendance.index') }}"
               class="px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-lg shadow-sm hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-colors">
                <svg class="inline h-4 w-4 ml-1" fill="currentColor" viewBox="0 0 20 20">
                    <path d="M3 5a2 2 0 012-2h10a2 2 0 012 2v2H3V5zm0 4h14v6a2 2 0 01-2 2H5a2 2 0 01-2-2V9zm3 2a1 1 0 100 2h6a1 1 0 100-2H6z"></path>
                </svg>
                الحضور
            </a>
        </nav>
        <div class="flex items-center gap-2">
            @auth
                ...
                <form method="POST" action="{{ route('logout') }}" class="inline">
                    @csrf
                    <button type="submit" class="px-3 py-1.5 text-sm font-medium text-white bg-red-600 rounded-md hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 transition-colors">
                        تسجيل الخروج
                    </button>
                </form>
            @endauth
            @guest
                <a href="{{ route('login') }}" class="px-3 py-1.5 text-sm font-medium text-white bg-blue-600 rounded-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-colors">
                    تسجيل الدخول
                </a>
            @endguest
        </div>
