...
<div class="min-h-screen bg-gray-50 py-8" dir="rtl">
    <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
        <!-- Header -->
        <div class="mb-8">
            <h1 class="text-3xl font-bold text-gray-900 mb-2">تعديل بيانات الطالب</h1>
            <nav class="flex" aria-label="Breadcrumb">
                <ol class="flex items-center space-x-reverse space-x-4">
                    ...
                    <li class="flex items-center">
                        ...
                        <span class="text-gray-500">تعديل بيانات {{ $student->name }}</span>
                    </li>
                </ol>
            </nav>
        </div>
        ...
        <div class="bg-white shadow-lg rounded-lg">
            <form method="post" action="{{route('student.update',$student)}}" class="p-6 space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <input type="text" name="mobile" id="mobile"
                           ...
                           value="{{ old('mobile', $student->mobile) }}" placeholder="أدخل رقم الجوال">
                    @error('mobile')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                    ...
                    <div class="flex space-x-reverse space-x-6">
                        <div class="flex items-center">
                            <input type="radio" name="gender" value="male" id="male"
                                   ...
                                   @checked(old('gender', $student->gender) == 'male')>
                            <label for="male" class="mr-2 text-sm text-gray-700">ذكر</label>
                        </div>
                        <div class="flex items-center">
                            <input type="radio" name="gender" value="female" id="female"
                                   class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300"
                                   @checked(old('gender', $student->gender) == 'female')>
                ...
                <div>
                    ...
                    <input type="number" name="age" id="age"
                           ...
                           value="{{ old('age', $student->age) }}" placeholder="أدخل عمر الطالب" min="1" max="100">
                    @error('age')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
