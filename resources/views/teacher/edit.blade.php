...
@section('content')
    <div class="min-h-screen bg-gray-50 py-8" dir="rtl">
        <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
            <!-- Header -->
            <div class="mb-8">
                <h1 class="text-3xl font-bold text-gray-900 mb-2">تعديل بيانات المعلم</h1>
                <nav class="flex" aria-label="Breadcrumb">
                    <ol class="flex items-center space-x-reverse space-x-4">
                        ...
                        <li class="flex items-center">
                            ...
                            <span class="text-gray-500">تعديل بيانات {{ $teacher->name }}</span>
                        </li>
                    </ol>
                </nav>
            </div>
            ...
            <div class="bg-white shadow-lg rounded-lg">
                <form method="post" action="{{route('teacher.update',$teacher)}}" class="p-6 space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div>
                        ...
                        <input type="text" name="name" id="name"
                               ...
                               value="{{ old('name', $teacher->name) }}" placeholder="أدخل اسم المعلم">
                        @error('name')
                    ...
                    <div>
                        ...
                        <input type="email" name="email" id="email"
                               ...
                               value="{{ old('email', $teacher->email) }}" placeholder="أدخل البريد الإلكتروني">
                        @error('email')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="mobile" id="mobile"
                               ...
                               value="{{ old('mobile', $teacher->mobile) }}" placeholder="أدخل رقم الجوال">
                        @error('mobile')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                        ...
                        <div class="flex space-x-reverse space-x-6">
                            <div class="flex items-center">
                                <input type="radio" name="gender" value="male" id="male"
                                    ...
                                    @checked(old('gender', $teacher->gender) == 'male')>
                                <label for="male" class="mr-2 text-sm text-gray-700">ذكر</label>
                            </div>
                            <div class="flex items-center">
                                <input type="radio" name="gender" value="female" id="female"
                                       class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300"
                                    @checked(old('gender', $teacher->gender) == 'female')>
                    ...
                    <div>
                        ...
                        <input type="number" name="age" id="age"
                               ...
                               value="{{ old('age', $teacher->age) }}" placeholder="أدخل عمر المعلم" min="22" max="70">
                        @error('age')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <select name="subject" id="subject"
                            ...
                            <option value="الرياضيات" @selected(old('subject', $teacher->subject) == 'الرياضيات')>الرياضيات</option>
                            <option value="العلوم" @selected(old('subject', $teacher->subject) == 'العلوم')>العلوم</option>
                            <option value="اللغة العربية" @selected(old('subject', $teacher->subject) == 'اللغة العربية')>اللغة العربية</option>
                            <option value="اللغة الإنجليزية" @selected(old('subject', $teacher->subject) == 'اللغة الإنجليزية')>اللغة الإنجليزية</option>
                            <option value="التاريخ" @selected(old('subject', $teacher->subject) == 'التاريخ')>التاريخ</option>
                            <option value="الجغرافيا" @selected(old('subject', $teacher->subject) == 'الجغرافيا')>الجغرافيا</option>
                            <option value="الفيزياء" @selected(old('subject', $teacher->subject) == 'الفيزياء')>الفيزياء</option>
                            <option value="الكيمياء" @selected(old('subject', $teacher->subject) == 'الكيمياء')>الكيمياء</option>
                            <option value="الأحياء" @selected(old('subject', $teacher->subject) == 'الأحياء')>الأحياء</option>
                            <option value="التربية الإسلامية" @selected(old('subject', $teacher->subject) == 'التربية الإسلامية')>التربية الإسلامية</option>
                            <option value="التربية الفنية" @selected(old('subject', $teacher->subject) == 'التربية الفنية')>التربية الفنية</option>
                            <option value="التربية الرياضية" @selected(old('subject', $teacher->subject) == 'التربية الرياضية')>التربية الرياضية</option>
                        </select>
                        @error('subject')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="number" name="salary" id="salary" step="0.01"
                               ...
                               value="{{ old('salary', $teacher->salary) }}" placeholder="أدخل الراتب" min="0">
                        @error('salary')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
