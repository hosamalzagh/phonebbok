        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-8">
                <div class="flex items-center justify-between">
                    <div>
                        <h1 class="text-3xl font-bold text-gray-900">قائمة المعلمين</h1>
                        <p class="mt-2 text-sm text-gray-600">إدارة بيانات المعلمين في النظام</p>
                    </div>
                                ...
                                <div class="px-4 py-4 sm:px-6 hover:bg-gray-50 transition-colors">
                                    <div class="flex items-center justify-between">
                                        <div class="flex items-center">
                                            ...
                                            <div class="mr-4 min-w-0 flex-1">
                                                <div class="flex items-center">
                                                    ...
                                                    <span class="mr-2 inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $teacher->gender == 'male' ? 'bg-blue-100 text-blue-800' : 'bg-pink-100 text-pink-800' }}">
                                                    {{ $teacher->gender == 'male' ? 'ذكر' : 'أنثى' }}
                                                </span>
                                                </div>
                                                <div class="flex items-center mt-1 flex-wrap">
                                                    <p class="text-sm text-gray-500 ml-2">
                                                        <span class="font-medium">المادة:</span> {{ $teacher->subject }}
                                                    </p>
                                                    <span class="mx-1 text-gray-300">•</span>
                                                    <p class="text-sm text-gray-500 ml-2">
                                                        <span class="font-medium">الجوال:</span> {{ $teacher->mobile }}
                                                    </p>
                                                    <span class="mx-1 text-gray-300">•</span>
                                                    <p class="text-sm text-gray-500">
                                                        <span class="font-medium">العمر:</span> {{ $teacher->age }} سنة
                                                    </p>
                                                </div>
                                                <div class="mt-1">
                                                    <p class="text-sm text-gray-600">
                                                        <span class="font-medium">البريد:</span> {{ $teacher->email }}
                                                    </p>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <span class="text-sm font-medium text-green-600 bg-green-50 px-2 py-1 rounded">
                                            {{ number_format($teacher->salary, 0) }} ج.م
                                        </span>
                                            <a href="{{ route('teacher.show', $teacher) }}"
                                            ...
                                            <form method="post" action="{{ route('teacher.destroy', $teacher) }}" class="inline-block"
                                                  onsubmit="return confirm('هل أنت متأكد من حذف بيانات هذا المعلم؟')">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"
            ...
            @else
                ...
                <div class="text-center py-12">
                    ...
                    <h3 class="mt-2 text-sm font-medium text-gray-900">لا توجد بيانات معلمين</h3>
                    <p class="mt-1 text-sm text-gray-500">ابدأ بإضافة أول معلم في النظام</p>
                    <div class="mt-6">
                        <a href="{{ route('teacher.create') }}"
